test_that("Cache Clearing works for memoised functions", {
  ...
  expect_true(dir.exists(file.path(get_options('f1dataR.cache'), 'f1dataR_http_cache')))
  clear_f1_cache()
  expect_false(memoise::has_cache(load_schedule)())
  expect_false(dir.exists(file.path(get_options('f1dataR.cache'), 'f1dataR_http_cache')))
