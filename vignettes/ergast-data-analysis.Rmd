...
We can make multiple repeat calls to the same function (with the same arguements) as the `f1dataR` package automatically caches responses from Ergast. You'll see this taken advantage of in a few areas.
...
## Grid to Finish Position Corellation
...
ggplot(results, aes(x = position, y = grid)) +
  ...
  ylab("Grid Position")
```
Of course, this isn't an interesting plot for a single race. Naturally we expect that a better grid position yields a better finish position, but there's so much variation in one race (including the effect of DNF) that it's a very weak correlation. We can look at the whole season instead:

```{r grid_to_finish_season, message=FALSE}
# Load the data
results<-list_rbind(map(seq_len(17), .f = function(x){Sys.sleep(1);load_results(2022, x)})) %>%
  mutate(grid = as.numeric(grid),
         position = as.numeric(position)) %>%
  filter(status %in% c('Finished', '+1 Lap', '+2 Laps', '+6 Laps')) %>%
  filter(grid > 0)
...
We can look at a scatterplot of a driver's laptimes throughout a race - possibly observing the effect of fuel usage, tire wear, pit stops, and race conditions. We'll also show extracting constructor colour from the built-in dataset.
...
With the above data, we can also visualize all driver's laptimes with violin plots. We'll trim the laptimes to exclude anything above 105 seconds to make the variation in laptime easier to see (i.e. show only racing laps).
